 ...
 *
 * Personal pages — accessible to any authenticated user. We mirror
 * the messages layout: resolve the user's remembered workspace via
 * the `tly-workspace` cookie and render the matching sidebar so a
 * landlord opening their notifications doesn't lose their nav.
 *
 * Without this restoration the workspace sidebar disappears the
 * moment a landlord clicks the bell.
 */
...
function pickWorkspace(
  ...
  const firstOrg = availability.orgs[0];
  if (firstOrg) {
    return { kind: 'landlord', slug: firstOrg.slug };
  }
